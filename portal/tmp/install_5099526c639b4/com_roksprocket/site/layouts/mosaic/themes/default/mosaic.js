...
}if(typeOf(d)=="string"){d=document.getElement(d);}return d.get("data-mosaic");},loadMore:function(e,d,g){d=this.getContainer(d);g=(typeOf(g)=="number")?g:this.getSettings(d).page||1;
if(!d.retrieve("roksprocket:mosaic:attached")){return;}var f=d.retrieve("roksprocket:mosaic:ajax"),i=d.getElement("[data-mosaic-filterby].active"),h={moduleid:d.get("data-mosaic"),behavior:!g?"reset":"append",filter:i?i.get("data-mosaic-filterby")||"all":"all",page:++g};
...
}this.detach(e);},onSuccess:function(k,f){var g="id-"+this.getID(f),p=f.retrieve("roksprocket:mosaic:ajax"),o=f.getElement("[data-mosaic-items]"),l=k.getPath("payload.html"),m=k.getPath("payload.page"),n=k.getPath("payload.more"),e=k.getPath("payload.behavior"),j=this.getSettings(f),r=j.animations,i;
this.setSettings(f,{page:(e=="reset"?1:m)});f.removeClass("refreshing");var h=new Element("div",{html:l}),d=h.getChildren(),q={},s={};q=this.getAnimation(f,"_set").style;
moofx(d).style(q);o.adopt(d);i=new Elements(d.getElements("img").flatten());this._loadImages(i.get("src"),function(){if(e=="reset"){this.mosaic[g].bricks.each(function(u,t){(function(){q=this.getAnimation(f,"_out");
moofx(u).style(q.style);s=Object.merge({},this.curve,{duration:"250ms",callback:function(){u.dispose();}});moofx(u).animate(q.animate,s);}).delay(t*50,this);
},this);}this.mosaic[g][e](d,function(){loadmore=f.getElements("[data-mosaic-loadmore]");if(loadmore){loadmore.removeClass("loader");}d=this.mosaic[g].bricks.filter(function(t){return d.contains(t);
});d.each(function(u,t){(function(){q=this.getAnimation(f,"_in");s=Object.merge({},this.curve,{curve:"cubic-bezier(0.37,0.61,0.59,0.87)",duration:"300ms"});
moofx(u).animate(q.animate,s);}).delay(t*100,this);},this);this.attach(f);f.getElements("[data-mosaic-loadmore]").removeClass("load-all")[!n?"addClass":"removeClass"]("hide");
...
d.getElements("[data-mosaic-orderby]").removeClass("active");if(g!="random"){e.addClass("active");}},initializeMosaic:function(e,k){var j="id-"+this.getID(e),d;
if(this.mosaic&&this.mosaic[j]){if(typeof k=="function"){k.call(this.mosaic[j].bricks);}d=e.getElements("[data-mosaic-loadmore]");if(d){d.removeClass("loader");
}return this.mosaic[j];}var i=e.getElements("img"),g=e.getElement("[data-mosaic-items]"),h=e.getElement(".active[data-mosaic-orderby]"),f={container:e,animated:true,gutter:0,order:h?h.get("data-mosaic-orderby"):(e.getElements("[data-mosaic-orderby]").length?"random":"default")};
if(k&&typeof k=="function"){f.callback=k;}moofx(g).style({"transform-style":"preserve-3d","backface-visibility":"hidden",opacity:1});moofx(g.getElements("[data-mosaic-item]")).style(this.getAnimation(e,"_in").animate);
if(!i.length){d=e.getElements("[data-mosaic-loadmore]");if(d){d.removeClass("loader");}this.mosaic[j]=new RokSprocket.MosaicBuilder(g,f);}else{this._loadImages(i.get("src"),function(){d=e.getElements("[data-mosaic-loadmore]");
if(d){d.removeClass("loader");}this.mosaic[j]=new RokSprocket.MosaicBuilder(g,f);}.bind(this));}return this.mosaic[j];},_loadImages:function(d,e){return d.length?new Asset.images(d,{onComplete:e.bind(this)}):e.bind(this)();
}});var c=new Class({Implements:[Options,Events],options:{container:null,resizeable:false,animated:false,gutter:0,fitwidth:false,order:"default",containerStyle:{position:"relative"}},initialize:function(e,d){this.setOptions(d);
...
var l,p,f,n;l=Math.ceil(j.offsetWidth/(this.columnWidth+this.options.gutter));l=Math.min(l,this.cols);if(l==1){f=this.colYs;}else{p=this.cols+1-l;f=[];
