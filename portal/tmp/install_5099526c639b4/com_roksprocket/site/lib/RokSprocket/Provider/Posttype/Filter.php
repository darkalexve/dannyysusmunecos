...
class RokSprocket_Provider_Posttype_Filter extends RokSprocket_Provider_AbstractWordpressPlatformFilter
{
	...
	protected function setBaseQuery()
	{
        ...
        foreach($taxonomies as $taxonomy){
            //join over taxonomy
            $i++;
            $this->base_query .= ' LEFT JOIN '.$wpdb->term_relationships.' as tr'.$i.' ON tr'.$i.'.object_id = p.ID';
            $this->base_query .= ' LEFT JOIN '.$wpdb->term_taxonomy.' as tx'.$i.' ON (tx'.$i.'.term_taxonomy_id = tr'.$i.'.term_taxonomy_id AND tx'.$i.'.taxonomy = "'.$taxonomy.'")';
            $this->base_query .= ' LEFT JOIN '.$wpdb->terms.' as t'.$i.' ON t'.$i.'.term_id = tx'.$i.'.term_id';
        }
        unset($i);

        //join over users
        $this->base_query .= ' LEFT JOIN '.$wpdb->users.' as u ON u.ID = p.post_author';
